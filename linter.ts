...
const loadConfig = async (mode: string) => {
  ...
  // --- expander is no longer supported (see docs/per-finding-workflow.md) ---
  for (const entry of config.checks || []) {
    if (entry && Object.prototype.hasOwnProperty.call(entry, "expander")) {
      throw new Error(
        `Check "${entry.name ?? entry.export ?? "?"}" has an "expander" block. ` +
        `Expanders and virtual entries have been removed; each check now runs directly on a file path. ` +
        `Remove the "expander" field from linter-config.json. ` +
        `See docs/per-finding-workflow.md for the new per-finding workflow.`
      );
    }
  }
...
const runChecks = async (files: string[], checks: BaseCheck[], { lintOnly = false, verbose = false, ...deps }: Record<string, unknown>) => {
  ...
  const runOne = async (check: BaseCheck, file: string): Promise<LinterCheckResult> => {
    if (lintOnly) {
      return check.lint(file, deps);
    }
    const combined = await check.lintAndFix(file, deps);
    return combined ?? await check.fix(file, deps);
  };

  const processFile = async (file: string, fileChecks: BaseCheck[]) => {
    const results: { res: LinterCheckResult; checkName: string }[] = [];
    for (const check of fileChecks) {
      let res: LinterCheckResult;
      try {
        res = await runOne(check, file);
      } catch (err) {
