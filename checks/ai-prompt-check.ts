...
/**
 * AI Prompt check — pure string-in / string-out on a whole file.
 *
 * Reads the target file, sends its content to the configured AI provider,
 * and writes any returned content back verbatim on a fix.
 *
 ...
 *                    keyed by relative file path and content hash.
...
export class AiPromptCheck extends BaseCheck {
  ...
  override async lint(file: string, _deps: Record<string, unknown>): Promise<CheckResult> {
    ...
    try {
      ...
      return { status: "error", output: `cannot read file: ${message}` };
    }
  ...
  async #writeLock(lockKey: string, content: string): Promise<void> {
  ...
  static override getHelp(): { name: string; description: string; options: string } {
    return {
      name: "AiPromptCheck",
      description:
        "Invokes an AI provider with a user-defined prompt against a whole file. " +
        "The check reads the file content, sends it to the AI along with the lint " +
        "or fix instruction, and writes any returned content back on a fix.",
      options:
        "aiProvider — which AI provider to use: 'claude' (default) or 'gemini'; " +
