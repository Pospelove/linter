 ...
 */
const normalizeFindings = async (file, checkName, res) => {
    if (res.status === "pass") {
...
const resolveClass = async (entry) => {
    ...
    if (!Cls) {
        throw new Error(`Export "${exportName}" not found in built-in registry. ` +
...
const loadConfig = async (mode) => {
    ...
    for (const entry of config.checks) {
        if (entry.expander !== undefined) {
            throw new Error(`Check "${entry.name}" has an "expander" block, but expanders have been removed. ` +
...
const runChecks = async (files, checks, { lintOnly = false, verbose = false, ...deps }) => {
    ...
    const emitResults = (file, results) => {
    ...
    if (lintOnly) {
        ...
        await Promise.all(groupedWork.map(({ file, checks }) => limit(async () => {
        ...
        for (const { file, checks } of groupedWork) {
            ...
            for (const check of checks) {
                try {
                    ...
                    if (res.extraFiles)
                        res.extraFiles.forEach((f) => extraFiles.add(f));
                    const findings = await normalizeFindings(file, check.name, res);
    ...
    if (counters.pass > 0)
        parts.push(`${counters.pass} passed`);
    if (counters.fixed > 0)
        parts.push(`${counters.fixed} fixed`);
    if (counters.fail > 0)
        parts.push(`${counters.fail} failed`);
    if (counters.error > 0)
        parts.push(`${counters.error} errored`);
...
const upgrade = () => {
    ...
    switch (method) {
        ...
        case "single-file": {
            ...
            try {
                execSync(`curl -fSL --retry 3 --retry-delay 5 -o "${tmpPath}" "${UPGRADE_URL}"`, { stdio: "inherit" });
            }
            catch {
                try {
...
const buildPrd = (failedPairs, prdConfig, checkEntries, baseCommand) => {
    ...
    for (const { file, checkName, finding } of failedPairs) {
        if (!byCheck.has(checkName))
            byCheck.set(checkName, []);
    ...
    for (const entry of checkEntries || []) {
        if (entry.prd?.group) {
            if (!groupToAllCheckNames.has(entry.prd.group))
                groupToAllCheckNames.set(entry.prd.group, []);
            groupToAllCheckNames.get(entry.prd.group).push(entry.name);
        }
    }
    // Separate checks into prd-grouped vs ungrouped
    const prdGroups = new Map(); // groupName -> [{ checkName, entries, checkPrd }]
    const ungroupedChecks = [];
    for (const [checkName, entries] of sortedChecks) {
        const checkPrd = checkPrdMap[checkName] || {};
        if (checkPrd.group) {
            if (!prdGroups.has(checkPrd.group))
                prdGroups.set(checkPrd.group, []);
    ...
    for (const [groupName, members] of [...prdGroups.entries()].sort(([a], [b]) => a.localeCompare(b))) {
        ...
        for (let i = 0; i < allFiles.length; i += filesPerStory) {
            ...
            const applyGroupPlaceholders = (str) => str
            ...
            const mainCriteria = allGroupCheckNames.length > 0
                ...
                : [];
            const fingerprints = members.flatMap((m) => m.entries
    ...
    for (const { checkName, entries, checkPrd } of ungroupedChecks) {
        ...
        for (let i = 0; i < entries.length; i += filesPerStory) {
            ...
            const applyPlaceholders = (str) => str
...
(async () => {
    ...
    try {
        ...
        if (mode === "hook") {
            ...
            allFiles.forEach((file) => ensureCleanExit(spawnSync("git", ["add", file], { stdio: "inherit", encoding: "utf-8" })));
        }
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
