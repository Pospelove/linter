...
export class AllFilesSource extends BaseFileSource {
    ...
    constructor(repoRoot, options = {}) {
        ...
        this.#includePatterns = Array.isArray(include)
            ? include.filter((i) => typeof i === "string")
            : typeof include === "string"
                ? [include]
                : [];
        const exclude = options["exclude"];
        this.#excludePatterns = Array.isArray(exclude)
            ? exclude.filter((i) => typeof i === "string")
    ...
    async resolve() {
        ...
        const files = output
            ...
            .filter((f) => f.trim() !== "")
            .filter((rel) => {
            if (this.#includePatterns.length > 0 && !this.#includePatterns.some((p) => matchGlob(p, rel)))
                return false;
            if (this.#excludePatterns.some((p) => matchGlob(p, rel)))
                return false;
            return true;
        })
