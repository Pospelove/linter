...
export class LocalizationKeyCheck extends BaseCheck {
    ...
    constructor(repoRoot, options = {}) {
        ...
        this.#registrationFiles = registrationFiles.map((f) => path.resolve(repoRoot, String(f)));
    ...
    async appliesTo(file) {
        ...
        if (this.#excludeRegistrationFiles) {
            ...
            if (this.#registrationFiles.some((r) => r === abs))
                return false;
        }
        return true;
    }
    async #loadRegistry() {
        ...
        if (this.#registryPattern !== null) {
            ...
            re = new RegExp(`${fn}\\s*\\(\\s*(?:"([^"]*)"\\s*,\\s*"([^"]*)"|'([^']*)'\\s*,\\s*'([^']*)')`, "g");
        }
        for (const filePath of this.#registrationFiles) {
            ...
            try {
                ...
                this.#registryErrors.push(`cannot read registration file ${path.relative(this.repoRoot, filePath)}: ${msg}`);
                continue;
            }
            let m;
